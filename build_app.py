...
for r in data['cluster_table']:
 m=obs['leiden'].astype(str).eq(r['cluster'])
...
function values(v){return v==='leiden'?D.clusters.map(Number):D.obs[v]||D.genes[v]}
function plot(selected=null){let c=colour.value, vals=values(c), colors=selected?D.clusters.map(x=>x===selected?'#d62728':'#c8c8c8'):vals; Plotly.react('plot',[{x:D.umap.map(x=>x[0]),y:D.umap.map(x=>x[1]),mode:'markers',type:'scattergl',marker:{size:6,color:colors,colorscale:'Viridis',showscale:!selected,colorbar:{title:c}},text:D.clusters.map(x=>'cluster '+x),hovertemplate:'%{text}<extra></extra>'}],{title:'UMAP',xaxis:{title:'UMAP 1'},yaxis:{title:'UMAP 2'},margin:{t:45}})}
